...
fn create_file_content(parent_idx: usize, name: &[u8], content: &[u8]) {
    let inode_idx = allocate_inode().expect("No inodes");
    let needed_blocks = if content.len() == 0 { 1 } else { (content.len() + 4095) / 4096 };
    let mut block_indices = [0usize; 16];
    for i in 0..needed_blocks {
        block_indices[i] = allocate_block().expect("No blocks");
    }
    {
        let inode = get_inode(inode_idx).unwrap();
        inode.file_type = FileType::File;
        inode.size = content.len() as u32;
        inode.block = block_indices[0] as u32;
        inode.parent = parent_idx as u32;
    }
    if content.len() > 0 {
        let mut block_idx = 0;
        ...
        while let Some(chunk) = chunks.next() {
            write_block(block_indices[block_idx], chunk);
            block_idx += 1;
        }
    }
    {
...
pub fn init() {
    ...
    let _backpack = create_dir(root_inode_idx, b"BACKPACK");

    create_file_content(whip, b"ls.mom", include_bytes!("../../../userspace/ls.mom"));
    create_file_content(whip, b"echo.mom", include_bytes!("../../../userspace/echo.mom"));
    create_file_content(whip, b"clear.mom", include_bytes!("../../../userspace/clear.mom"));
    create_file_content(toys, b"hello.mom", include_bytes!("../../../userspace/hello.mom"));
    create_file_content(whip, b"shutdown.mom", include_bytes!("../../../userspace/shutdown.mom"));
    create_file_content(toys, b"momstats.mom", include_bytes!("../../../userspace/momstats.mom"));

    print(b"Ramdisk Storage Ready.\n");
}
pub fn verify_fs() {
    ...
    unsafe {
        ...
        read_block(root.block as usize, &mut buffer);
