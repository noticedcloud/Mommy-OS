...
use super::{get_my_mac, GATEWAY_MAC, MY_IP};
...
pub unsafe fn resolve_hostname(hostname: &str) -> Option<[u8; 4]> {
    ...
    loop {
        if let Some(ip) = core::ptr::read_volatile(&raw const RESOLVED_IP) {
            DNS_WAITING = false;
            return Some(ip);
        }
...
unsafe fn send_dns_query(hostname: &str) {
    ...
    let _offset = eth_len + ip_len + udp_len + dns_header_len;
    ...
    while idx < data.len() {
        ...
        if type_code == 1 && rd_len == 4 {
                if idx + 4 <= data.len() {
                    ...
                    crate::serial::print_serial(b"[DNS] IP Solved!\n");
                    RESOLVED_IP = Some(ip);
                    return;
                }
        }
