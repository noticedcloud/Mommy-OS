...
pub fn put_pixel(x: u16, y: u16, color: u32) {
    unsafe {
        ...
        let addr = (VESA_INFO.framebuffer_addr + offset) as *mut u32;
        if VESA_INFO.bpp == 32 {
            addr.write_volatile(color | 0xFF000000);
        }
    }
}
pub fn init() {
    unsafe {
        ...
        if VESA_INFO.width > 0 {
            let fb_start = VESA_INFO.framebuffer_addr;
            let fb_size = (VESA_INFO.height as u64 * VESA_INFO.pitch as u64) as u64;
            let fb_end = fb_start + fb_size;

            crate::serial::print_serial(b"[VESA] Mapping Framebuffer: ");
            crate::serial::print_hex(fb_start);
            crate::serial::print_serial(b" - ");
            crate::serial::print_hex(fb_end);
            crate::serial::print_serial(b"\n");

            let page_size = 4096;
            for addr in (fb_start..fb_end + page_size).step_by(page_size as usize) {
